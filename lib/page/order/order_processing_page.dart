...
class OrderProcessingPage extends ConsumerStatefulWidget {
  const OrderProcessingPage({super.key});

  @override
  ConsumerState<OrderProcessingPage> createState() => _OrderProcessingPageState();
}

class _OrderProcessingPageState extends ConsumerState<OrderProcessingPage> {
  OrderTab _tab = OrderTab.processing;
                  ...
                  Expanded(
                    child: uid == null || uid.isEmpty
                      ...
                      builder: (context, snap) {
                        ...
                        final isProcessingTab = _tab == OrderTab.processing;

                        return SingleChildScrollView(
                          padding: const EdgeInsets.fromLTRB(12, 0, 12, 12),
                          child: OrderList(
                            orders: orders,
                            actionLabel: isProcessingTab ? 'Hủy đơn hàng' : 'Xem chi tiết',
                            onAction: (o) {
                              if (isProcessingTab) {
                                _cancelOrder(o);
                              } else {
                                context.push('/orders/detail/${o.orderId}');
                              }
                            },
                            onSeeMore: (o) {
                              context.push('/orders/detail/${o.orderId}');
